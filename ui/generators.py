...
## 使用するスタイルガイド
コンテキスト内の「{style_guide_name}」に従って台本を作成してください。
...
def generate_script(
    ...
    folder: ContentFolder,
    video_type: str = "long",
    script_type: str = "filming",
) -> str:
    """タイトルとフォルダコンテキストから台本を生成する。

    Args:
        script_type: "filming"（撮影用）または "slide"（スライド作成用）。ロング動画のみ有効。
    """
    context = folder.get_context()
    if video_type == "short":
        ...
        style_guide_name = (
            "スライド作成用台本スタイルガイド" if script_type == "slide" else "撮影用台本スタイルガイド"
        )
        ...
        max_tokens = 4096 if script_type == "slide" else 8192
    agent = _SimpleAgent()
...
def save_script(
    ...
    if not slug:
        return
    filename = "script_slide.txt" if script_type == "slide" else "script_filming.txt"
    try:
        _get_storage(folder, video_type).save_script(slug, script, filename=filename)
    except Exception as e:
